...
/** Instantiate a new experiment to use the ez_exp module.
* @func
...
*/
EzExp.loadExperiment =  function(inputFilePath, participantId, participantColName='Participant', trialId = 0) {
  var exp = new this.Experiment(inputFilePath, participantId, participantColName, trialId);
...
EzExp.Experiment = function (ifp, pid, pcn, tid) {
  ...
  */
  this.loadNextTrial = function() {
    ...
    if (this._trials.length <= this._currentTrialId) { console.error("No more trial to load."); return null; }
  ...
  */
  this.saveCurrentTrial = function() {
    ...
    var data = "";
    var pdata = t.getParameterData();
    if(pdata) { data += pdata.join(','); }
    ...
    fs.appendFile(this._outputFilePath, data+'\n', 'utf8', (err) => { if (err) { throw err; } });
  };
};
