...
EzExp.Trial = function(parameterData) {
  ...
  */
  this.getParameterData = function(index) {
    if(index!==undefined) { return this._parameterData[index]; }
    else { return this._parameterData; }
  };
  ...
  this.startTrial = function() {
    if(this._trialState !== EzExp.Enums.TrialState.NotStarted) { console.log('Trial already started'); return false; }
    this._mainTimer.start();
    this._trialState = EzExp.Enums.TrialState.Started;
    return true;
  };
  ...
  this.endTrial = function() {
    if(this._trialState === EzExp.Enums.TrialState.NotStarted) { console.log('Trial never started'); return false; }
    else if(this._trialState === EzExp.Enums.TrialState.Ended) { console.log('Trial already stopped'); return false; }
    // stop all timers
    for(var t in this._timers) { if(this._timers[t].getState() != EzExp.Enums.TimerState.Stopped) { this._timers[t].stop(); } }
    this._mainTimer.stop();
    this._trialState = EzExp.Enums.TrialState.Ended;
    return true;
  };
  ...
  */
  this.addTimer = function(tag) {
    if(this._timers[tag]) { return false; }
    else { this._timers[tag] = new EzExp.Timer(); return true; }
  };
  ...
  */
  this.removeTimer = function(tag) {
    if(this._timers[tag]) { delete this._timers[tag]; return true; }
    else { return false; }
  };
  ...
  */
  this.startTimer = function(tag) {
    if(this._timers[tag]) { this._timers[tag].start(); return true; }
  ...
  */
  this.pauseTimer = function(tag) {
    if(this._timers[tag]) { this._timers[tag].pause(); return true; }
    else { console.error('Unknown timer tag ('+tag+')'); return false; }
  };
  ...
  */
  this.resumeTimer = function(tag) {
    if(this._timers[tag]) { this._timers[tag].resume(); return true; }
    else { console.error('Unknown timer tag ('+tag+')'); return false; }
  };
  ...
  */
  this.stopTimer = function(tag) {
    if(this._timers[tag]) { this._timers[tag].stop(); return true; }
    else { console.error('Unknown timer tag ('+tag+')'); return false; }
  };
  ...
  */
  this.getTimerDuration = function(tag) {
    if(this._timers[tag]) { return this._timers[tag].getDuration(); }
  ...
  */
  this.getTimerState = function(tag) {
    if(this._timers[tag]) { return this._timers[tag].getState(); }
  ...
  */
  this.getTimers = function() { return this._timers; }

  /** Get the overall duration of the trial.
  * @return {number} Duration of the timer (0 if not started already and -1 if the tag does not exist).
  */
  this.getMainDuration = function() { return this._mainTimer.getDuration(); };

  /** Get trial state (see {@link EzExp.Enums.TrialState|TrialState}).
  * @return {EzExp.Enums.TrialState} State of the trial.
  */
  this.getState = function() { return this._trialState; };
  ...
  */
  this.saveResultData = function(tag, data) { this._resultData[tag] = data; };
  ...
  */
  this.getResultData = function(tag) {
    if(tag) {
      if(this._resultData[tag]===undefined) { console.error('Unknown result tag ('+tag+')'); return null; }
      else { return this._resultData[tag]; }
    } else { return this._resultData; }
  };
};
